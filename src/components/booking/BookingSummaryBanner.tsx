      ...
      {selectedService && (
        <div className="relative mt-8 p-4 bg-white/10 backdrop-blur-md rounded-xl border border-white/20 space-y-2 text-xs font-semibold">
          ...
          <p className="flex justify-between gap-2">
            <span>บริการ:</span> <span className="truncate max-w-[120px]">{selectedService.nameTh || selectedService.name}</span>
          </p>
          {selectedStaff && (
            <p className="flex justify-between gap-2">
              <span>ช่าง:</span> <span className="truncate max-w-[120px]">{selectedStaff.name}</span>
            </p>
          )}
          {selectedDate && (
            <p className="flex justify-between gap-2">
              <span>วันที่:</span> <span>{selectedDate}</span>
            </p>
          )}
          {selectedTime && (
            <p className="flex justify-between gap-2">
              <span>เวลา:</span> <span>{selectedTime} น.</span>
            </p>
          )}
